...
BEGIN {
	...
	Function New-LocateScheduledTask  {
		...
		# Script to execute
		$updatefilename = """$locatepath\Update-LocateDB.ps1"""
		#if ($updatefilename -like "* *" -or $updatefilename -like "*'*") {
		#	# Task Manager does not handle spaces or apostrophes well, so it has to be encoded.
		#	write-warning "Illegal characters in commandline. Using encodedcommand"
